...
// stoptaskCmd represents the stoptask command
...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		execstoptask(viper.GetViper().GetString("server"), taskids)
	},
}

func init() {
	rootCmd.AddCommand(stoptaskCmd)
	stoptaskCmd.Flags().StringP("taskids", "i", "", "taskids to stop ,split by ','")
	stoptaskCmd.MarkFlagRequired("taskids")
}

func execstoptask(server string, ids string) {
